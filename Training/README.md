...
1.  **Validate Coordinate Extraction Locally:**
    ```bash
    uv run python3 Training/test_anthropogenic_covariates.py --test-extract --limit 3
    ```
...
2. **Run Full Pipeline & Upload:**
    ...
    ```bash
    nohup uv run python3 Training/test_anthropogenic_covariates.py --test-upload > anthropogenic_upload.log 2>&1 &
    ```
